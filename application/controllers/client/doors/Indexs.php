	...
	class Indexs extends Mains
	{
		...
		public function todays()
		{
			if(is_fulls("token"))
			{
				...
				$query=$this->db->query("select `start_time`,`end_time` from `dg_doors` where `uid`='".$rs["id"]."' and ((`start_time`>='$s' and `start_time`<='$e') AND (`end_time`<='$e' and `end_time`>='$s')) order by `id` desc");
