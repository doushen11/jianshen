	...
	class Message extends Mains
	{
		...
		public function indexs()
		{
			...
			if(is_fulls("token")){
				...
				$_array=array();
				$query1=$this->db->query("select count(id) as `alls` from `dg_message_u` where `uid`='".$rs["id"]."'");
				$result1=$query1->row_array();
				$result1["alls"]==""?$_array["all_count"]=0:$_array["all_count"]=$result1["alls"];
				$query2=$this->db->query("select count(id) as `alls` from `dg_message_u` where `uid`='".$rs["id"]."' and `read`='1'");
				$result2=$query2->row_array();
				$result2["alls"]==""?$_array["all_noread_count"]=0:$_array["all_noread_count"]=$result2["alls"];
				$query3=$this->db->query("select count(id) as `alls` from `dg_message_u` where `uid`='".$rs["id"]."' and `read`='2'");
		...
		public function pages()
		{
			...
			if(is_fulls("token"))
			{
				...
				if($read==0)
				{
					$sql="select `id`,`title`,`time`,`contents`,`read` from `dg_message_u` where `uid`='".$rs["id"]."' order by `id` desc";
				}
				else
				{
					$sql="select `id`,`title`,`time`,`contents`,`read` from `dg_message_u` where `uid`='".$rs["id"]."' and `read`='$read' order by `id` desc";	
				}
		...
		public function item()
		{
			if(is_fulls("token") && is_fulls("id",1))
			{
				...
				$id=I("id");
				$rs=$this->check_token($token);
				$query=$this->db->query("select `id`,`title`,`time`,`contents`,`read` from `dg_message_u` where `uid`='".$rs["id"]."' and `id`='$id'");
				if($query->num_rows()>0)
				{
					...
					{
						//更新阅读状态
						$this->db->query("update `dg_message_u` set `read`='2' where `id`='$id'");
					}
		...
		public function dels()
		{
			...
			if(is_fulls("token") && is_fulls("id"))
			{
				...
				$id=I("id");
				$rs=$this->check_token($token);
				$this->db->query("delete from `dg_message_u` where `id` in (".$id.") and `uid`='".$rs["id"]."'");
