...
<script>
	...
	{
		//alert(1);
		$.ajax({url:"<?php echo http_url();?>admin/notes/homes_ajaxs/" + pageindex + "?keywords=" + keywords + "&start_time=" + start_time + "&end_time=" + end_time, 
			type: 'POST', 
			dataType: 'html', 
			timeout: 15000, 
			error: function(){
	...
	function del_rs(id){
		if(form_status==0 || form_status==3){
			$.ajax({url:"<?php echo http_url();?>admin/notes/all_subs", 
			type: 'POST', 
			data:{id:id},
			dataType: 'html', 
			timeout: 15000, 
...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 意见反馈 <span class="c-gray en">&gt;</span> 教练端意见反馈 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
