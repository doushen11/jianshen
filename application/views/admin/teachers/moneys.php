...
<script>
	...
	{
		location='<?php echo http_url();?>admin/teachers/homes_edits/' + id + "?keywords=" + keywords + "&pageindex=" + pageindex + "&start_time=" + start_time + "&end_time=" + end_time + "&states=" + states;	
	}
	...
	{
		location='<?php echo http_url();?>admin/classs/caokes_add/' + id + "?keywords=" + keywords + "&pageindex=" + pageindex + "&start_time=" + start_time + "&end_time=" + end_time + "&states=" + states;	
	}
	...
	{
		//alert(1);
		$.ajax({url:"<?php echo http_url();?>admin/teachers/moneys_ajaxs/" + pageindex + "?keywords=" + keywords + "&start_time=" + start_time + "&end_time=" + end_time + "&states=" + states, 
			type: 'POST', 
			dataType: 'html', 
			timeout: 15000, 
			error: function(){
...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 教练管理 <span class="c-gray en">&gt;</span> 教练收益列表 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
<div class="page-container">
    ...
    <input type="text" onfocus="WdatePicker({maxDate:'#F{$dp.$D(\'datemax\')||\'%y-%M-%d\'}'})" id="datemin" class="input-text Wdate" style="width:120px;" value="<?php echo $start_time;?>">
        ...
        <select id="states" name="states" style="height:30px; line-height:30px; border:#e4e4e4 1px solid;">
            ...
            <option value="1" <?php if($states==1){?> selected<?php }?>>私课</option>
            <option value="2" <?php if($states==2){?> selected<?php }?>>操课</option>
        </select>
        &nbsp;&nbsp;
