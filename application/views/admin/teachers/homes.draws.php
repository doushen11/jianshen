...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 会员管理 <span class="c-gray en">&gt;</span> 给指定私课教练打款处理 <a id="shuaxins" class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i id="wysx" class="Hui-iconfont">&#xe68f;</i></a></nav>
<article class="page-container">
	<div class="form form-horizontal" id="form-article-add" action="#">
		...
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-2">教练手机号：</label>
			<div class="formControls col-xs-8 col-sm-9">
 		...
 		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" placeholder="" value="<?php echo $result["balance"];?>" id="balance" name="balance" disabled>
			</div>
		</div>
       
       <?php
       	$qy=$this->db->query("select `money`,`time` from `dg_teacher_money` where `tid`='".$result["id"]."' and `models`='2' order by `id` desc limit 1");
		if($qy->num_rows()>0){
			$rs=$qy->row_array();
	   ?>
       <div class="row cl">
			<label class="form-label col-xs-4 col-sm-2">上次提现：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<strong>时间：<?php echo date("Y-m-d H:i:s",$rs["time"]);?></strong>&nbsp;&nbsp;&nbsp;<strong style="color:#cc0000;">金额：<?php echo $rs["money"];?></strong>
			</div>
		</div>
       <?php }?>
        <div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" placeholder="" value="<?php echo $result["balance"];?>" id="moneys" name="moneys" disabled>
			</div>
		</div>
        ...
        <div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" placeholder="可以填写转到的银行相关等等..." value="" id="desc" name="desc">
			</div>
		</div>       

		<div class="row cl">
			<div class="col-xs-8 col-sm-9 col-xs-offset-4 col-sm-offset-2">
				<button onClick="article_save_submit();" class="btn btn-primary radius" type="submit"><i class="Hui-iconfont">&#xe632;</i> 确认提现</button>
...
<script>
	...
	function article_save_submit()
	{
		...
		if(form_loads==1)
		{
			...
			var balance=$("#balance").val().replace(/(^\s*)|(\s*$)/g,"");
			var moneys_test=/^\-?([1-9]\d*(\.\d{1,2})?|0\.\d{1,2})$|^0$/;
			if(!moneys_test.test(moneys))
			{
				layer.tips('请填写正确提现金额', '#moneys', {
				  tips: 3
				});			
			}
			else if(balance<moneys)
			{
				layer.tips('请填写正确提现金额', '#moneys', {
				  tips: 3
				});						
			...
			{
				$.ajax({url:"<?php echo http_url();?>admin/teachers/homes_draw_sub/<?php echo $result["id"];?>/<?php echo $result["balance_count"];?>", 
				type: 'POST', 
				data:{moneys:moneys,desc:desc}, 
				dataType: 'html', 
				timeout: 15000, 
					error: function(){
						layer.closeAll();
					...
					success:function(result){
						...
						if(result.indexOf("|")>0){
							...
							if(arr[0]==10000){
								...
								setTimeout("location='<?php echo http_url();?>admin/members/homes?keywords=<?php echo @$_GET["keywords"];?>&pageindex=<?php echo @$_GET["pageindex"];?>&start_time=<?php echo @$_GET["start_time"];?>&end_time=<?php echo @$_GET["end_time"];?>&states=<?php echo @$_GET["states"];?>'",1500);
							}else if(arr[0]==20000){
