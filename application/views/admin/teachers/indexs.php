...
<script>
	...
	{
		location='<?php echo http_url();?>admin/teachers/index_edits/' + id + "?keywords=" + keywords + "&pageindex=" + pageindex + "&start_time=" + start_time + "&end_time=" + end_time + "&states=" + states + "&doors=" + doors;	
	}
	
	//会员添加
	function member_add()
	{
		location='<?php echo http_url();?>admin/teachers/indexs_add/' + "" + "?keywords=" + keywords + "&pageindex=" + pageindex + "&start_time=" + start_time + "&end_time=" + end_time + "&states=" + states + "&doors=" + doors;	
	}	
	
	//会员添加
	function members_adds(id)
	{
		location='<?php echo http_url();?>admin/classs/sikes_add/' + id + "?keywords=" + keywords + "&pageindex=" + pageindex + "&start_time=" + start_time + "&end_time=" + end_time + "&states=" + states + "&doors=" + doors;	
	}
	
	//会员打款
	function members_draw(id)
	{
		location='<?php echo http_url();?>admin/teachers/sikes_draws/' + id + "?keywords=" + keywords + "&pageindex=" + pageindex + "&start_time=" + start_time + "&end_time=" + end_time + "&states=" + states + "&doors=" + doors;		
	}
	...
	{
		//alert(1);
		$.ajax({url:"<?php echo http_url();?>admin/teachers/indexs_ajaxs/" + pageindex + "?keywords=" + keywords + "&start_time=" + start_time + "&end_time=" + end_time + "&states=" + states + "&doors=" + doors, 
			type: 'POST', 
			dataType: 'html', 
			timeout: 15000, 
			error: function(){
	...
	function del_it(id){
		layer.confirm('警告：您确定要删除所选数据吗！如果当前教练下对应有已经发布的私课，系统会决绝您的删除操作！', {
			btn: ['确定','取消'] //按钮
		}, function(){
			del_rs(id);
		}, function(){
	...
	function del_rs(id){
		if(form_status==0 || form_status==3){
			$.ajax({url:"<?php echo http_url();?>admin/teachers/index_dels", 
			type: 'POST', 
			data:{id:id},
			dataType: 'html', 
			timeout: 15000, 
...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 教练管理 <span class="c-gray en">&gt;</span> 私课教练列表 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
<div class="page-container">
    ...
    <input type="text" onfocus="WdatePicker({maxDate:'#F{$dp.$D(\'datemax\')||\'%y-%M-%d\'}'})" id="datemin" class="input-text Wdate" style="width:120px;" value="<?php echo $start_time;?>">
		...
		<input type="text" class="input-text" style="width:250px" placeholder="教练ID/教练手机号/教练昵称/性别" id="keys" name="keys" value="<?php echo $keywords;?>">&nbsp;&nbsp;
        ...
        <select id="doors" name="doors" style="height:30px; line-height:30px; border:#e4e4e4 1px solid;">
        	<option value="">--请选择进门状态--</option>
            <option value="1" <?php if($doors==1){?> selected<?php }?>>门内</option>
            <option value="2" <?php if($doors==2){?> selected<?php }?>>门外</option>
        </select>
        &nbsp;&nbsp;
