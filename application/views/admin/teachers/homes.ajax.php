<div class="cl pd-5 bg-1 bk-gray mt-20"> <span class="l"><a href="javascript:del_all();" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i> 删除所选教练</a>  <a href="javascript:;" onclick="member_add()" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加操课教练</a></span> <span class="r">共有数据：<strong><?php echo $pageall;?></strong> 条</span> </div>
	<div class="mt-20">
    
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		<thead>
			<tr class="text-c">
			  <th width="21"><input type="checkbox" name="sx" id="sx" value="100" onClick="choose_all();" ></th>
			  <th width="35">教练ID</th>
				<th width="74">登录状态</th>
				<th width="85">进门出门</th>
				<th width="85">教练手机号</th>
				<th width="45">真实姓名</th>
				
				<th width="52">账户余额</th>
				<th width="71">目前累计课程</th>
				<th width="66">最后提现时间</th>
				<th width="65">注册时间</th>
				<th width="74">最后登录时间</th>
				<th width="68">最后登录IP</th>
				<th width="41">课程</th>
				<th width="64">操作</th>
            </tr>
		</thead>
		<tbody>
			...
			<tr id="zo_<?php echo $array["id"];?>" class="text-c" >
			  ...
			  <td><?php echo $array["id"];?></td>
				...
				<td><?php echo $array["balance_count"];?></td>
				<td>
				...
				<td><p><?php echo date("Y-m-d",$array["reg_time"]);?></p><p><?php echo date("H:i:s",$array["reg_time"]);?></p></td>
				<td>
                <?php
                	if($array["login_time"]>=$s_time && $array["login_time"]<=$e_time)
					{
				?>
                <strong style="color:#F00;"><p><?php echo date("Y-m-d",$array["login_time"]);?></p><p><?php echo date("H:i:s",$array["login_time"]);?></p></strong>
                <?php
					}
					else
					{
				?>
                <p>
                <?php echo date("Y-m-d",$array["login_time"]);?>
                </p>
                <p>
                <?php echo date("H:i:s",$array["login_time"]);?>
                </p>
                <?php
					}
				?>
				...
				<td class="f-14 td-manage"><a href="<?php echo http_url();?>admin/classs/caokes?keywords=<?php echo $array["mobile"];?>"><strong>查看</strong></a></td>
				<td class="f-14 td-manage"><a style="text-decoration:none" class="ml-5" onClick="members_edit('<?php echo $array["id"];?>')" href="javascript:;" title="编辑"><i class="Hui-iconfont">&#xe6df;</i></a>   <a style="text-decoration:none" class="ml-5" onClick="members_add('<?php echo $array["id"];?>')" href="javascript:;" title="发布操课"><i class="Hui-iconfont">&#xe6bf;</i></a>   <a style="text-decoration:none" class="ml-5" onClick="members_draw('<?php echo $array["id"];?>')" href="javascript:;" title="打款"><i class="Hui-iconfont">&#xe6ff;</i></a></td>
            </tr>
            <?php
            	}
			?>
