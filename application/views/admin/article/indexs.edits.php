...
<article class="page-container">
	<div class="form form-horizontal" id="form-article-add" action="#">
		...
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-2"><span class="c-red">*</span>相关内容：</label>
			<div class="formControls col-xs-8 col-sm-9"> 
				<textarea id="contents" name="contents" style="width:98%; height:120px; padding-left:1%; padding-right:1%; padding-top:1%; line-height:26px;"><?php echo stripslashes($result["contents"]);?></textarea>
			</div>
		</div>
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<strong style="color:#FF0000;">注意：不建议修改文章标题，除非是公告内容，以免修改后难以辨别</strong>
			</div>
		</div>        
		<div class="row cl">
...
<script>
	...
	function article_save_submit()
	{
		...
		if(form_loads==1)
		{
			...
			if(title=="")
			{
				layer.tips('请填写文章标题', '#title', {
				  tips: 3
				});			
			}
			else if(contents=="")
			{
				layer.tips('请填写文章内容', '#contents', {
				  tips: 3
				});
			}
			else
			{
				$.ajax({url:"<?php echo http_url();?>admin/article/edit_subs/<?php echo intval($this->uri->segment(4));?>", 
				type: 'POST', 
					...
					success:function(result){
						...
						if(result.indexOf("|")>0){
							...
							if(arr[0]==10000){
								...
								setTimeout("location='<?php echo http_url();?>admin/article/indexs'",1500);
							}else if(arr[0]==20000){
