...
<script>
	...
	{
		location='<?php echo http_url();?>admin/members/edits/' + id + "?keywords=" + keywords + "&pageindex=" + pageindex + "&start_time=" + start_time + "&end_time=" + end_time + "&states=" + states;	
	}
	...
	{
		//alert(1);
		$.ajax({url:"<?php echo http_url();?>admin/members/caokes_ajaxs/" + pageindex + "?keywords=" + keywords + "&start_time=" + start_time + "&end_time=" + end_time + "&states=" + states, 
			type: 'POST', 
			dataType: 'html', 
			timeout: 15000, 
			error: function(){
	...
	function del_it(id){
		layer.confirm('警告：您确定要删除所选数据吗！当前系统仅支持删除取消类记录，其他类型记录不支持删除！', {
			btn: ['确定','取消'] //按钮
		}, function(){
			del_rs(id);
		}, function(){
	...
	function del_rs(id){
		if(form_status==0 || form_status==3){
			$.ajax({url:"<?php echo http_url();?>admin/members/sikes_dels", 
			type: 'POST', 
			data:{id:id},
			dataType: 'html', 
			timeout: 15000, 
...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 会员管理 <span class="c-gray en">&gt;</span> 操课购买记录 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
<div class="page-container">
    ...
    <input type="text" onfocus="WdatePicker({maxDate:'#F{$dp.$D(\'datemax\')||\'%y-%M-%d\'}'})" id="datemin" class="input-text Wdate" style="width:120px;" value="<?php echo $start_time;?>">
		...
		<input type="text" class="input-text" style="width:250px" placeholder="课程节点ID/课程名称/会员手机号/会员昵称/教练手机号/教练真实姓名" id="keys" name="keys" value="<?php echo $keywords;?>">&nbsp;&nbsp;
        
        <select id="states" name="states" style="height:30px; line-height:30px; border:#e4e4e4 1px solid;">
        	<option value="">--请选择购买状态--</option>
            <option value="1" style="color:#09F;" <?php if($states==1){?> selected<?php }?>>待成立</option>
            <option value="2" style="color:#F0F;" <?php if($states==2){?> selected<?php }?>>已成立</option>
            <option value="3" style="color:green;" <?php if($states==3){?> selected<?php }?>>已完成</option>
            <option value="4" style="color:#999999;" <?php if($states==4){?> selected<?php }?>>用户取消</option>
            <option value="5" style="color:#999999;" <?php if($states==5){?> selected<?php }?>>系统取消</option>
        </select>
        &nbsp;&nbsp;
