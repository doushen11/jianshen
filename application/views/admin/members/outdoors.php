...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 会员管理 <span class="c-gray en">&gt;</span> 会员出门 <a id="shuaxins" class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i id="wysx" class="Hui-iconfont">&#xe68f;</i></a></nav>
<article class="page-container">
	<div class="form form-horizontal" id="form-article-add" action="#">
        ...
        <div class="row cl">
			<label class="form-label col-xs-4 col-sm-2">会员昵称：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" placeholder="" value="<?php echo $result["nickname"];?>" id="nickname" name="nickname" disabled>
			</div>
		</div>
       <div class="row cl">
			<label class="form-label col-xs-4 col-sm-2">上次进门时间：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<strong style="color:#F00;">
				<?php
					$querys=$this->db->query("select `start_time` from `dg_doors` where `uid`='".$result["id"]."' and `act`='1' order by `id` desc limit 1");
					//echo "select `start_time` from `dg_doors` where `uid`='".$result["id"]."' and `act`='3' order by `id` desc limit 1";
					$s_result=$querys->row_array(); 
					echo date("Y-m-d H:i:s",$s_result["start_time"]);
				?></strong>
			</div>
		</div>
       
       <div class="row cl">
			<label class="form-label col-xs-4 col-sm-2">出门时间：</label>
			<div class="formControls col-xs-8 col-sm-9">
			
                <select id="y1" name="y1" style="height:29px; line-height:29px;">
                    <option value="">--清选择年份--</option>
                    <?php
                    	for($i=2016;$i<=date("Y");$i++)
						{
					?>
                    <option value="<?php echo $i;?>" <?php if($i==date("Y")){?> selected<?php }?>><?php echo $i;?>年</option>
                    <?php
						}
					?>
                </select>
                &nbsp;&nbsp;
                <select id="m1" name="m1" style="height:29px; line-height:29px;">
                    <option value="">--清选择月份--</option>
                    <?php
                    	for($i=1;$i<=12;$i++)
						{
					?>
                    <option value="<?php echo $i;?>" <?php if($i==date("m")){?> selected<?php }?>><?php echo $i;?>月</option>
                    <?php
						}
					?>
                </select>
                &nbsp;&nbsp;
                <select id="d1" name="d1" style="height:29px; line-height:29px;">
                    <option value="">--清选择日--</option>
                    <?php
                    	for($i=1;$i<=31;$i++)
						{
					?>
                    <option value="<?php echo $i;?>" <?php if($i==date("d")){?> selected<?php }?>><?php echo $i;?>日</option>
                    <?php
						}
					?>
                </select>
                &nbsp;&nbsp;
                <select id="h1" name="h1" style="height:29px; line-height:29px;">
                    <option value="">--清选择小时--</option>
                    <?php
                    	for($i=0;$i<=23;$i++)
						{
					?>
                    <option value="<?php echo $i;?>" <?php if($i==date("H")){?> selected<?php }?>><?php echo $i;?>时</option>
                    <?php
						}
					?>
                </select>
                ...
                &nbsp;&nbsp;
                <select id="i1" name="i1" style="height:29px; line-height:29px;">
                    <option value="">--清选择分钟--</option>
                    <?php
                    	for($i=0;$i<=59;$i++)
						{
					?>
                    <option value="<?php echo $i;?>" <?php if($i==date("i")){?> selected<?php }?>><?php echo $i;?>分</option>
                    <?php
						}
					?>
                </select>
                &nbsp;&nbsp;
                <select id="s1" name="s1" style="height:29px; line-height:29px;">
                    <option value="">--清选择秒数--</option>
                    <?php
                    	for($i=0;$i<=59;$i++)
						{
					?>
                    <option value="<?php echo $i;?>" <?php if($i==date("s")){?> selected<?php }?>><?php echo $i;?>秒</option>
                    <?php
						}
					?>
                </select>
		...
		<div class="row cl">
			<div class="col-xs-8 col-sm-9 col-xs-offset-4 col-sm-offset-2">
				<button onClick="article_save_submit();" class="btn btn-primary radius" type="submit"><i class="Hui-iconfont">&#xe632;</i> 确 认 出 门 </button>
...
<script>
	...
	function article_save_submit()
	{
		...
		if(form_loads==1)
		{
			...
			var y1=$("#y1").val().replace(/(^\s*)|(\s*$)/g,"");
			var m1=$("#m1").val().replace(/(^\s*)|(\s*$)/g,"");
			var d1=$("#d1").val().replace(/(^\s*)|(\s*$)/g,"");
			var h1=$("#h1").val().replace(/(^\s*)|(\s*$)/g,"");
			var i1=$("#i1").val().replace(/(^\s*)|(\s*$)/g,"");
			var s1=$("#s1").val().replace(/(^\s*)|(\s*$)/g,"");
			
			//alert(tupian);
			if(y1=="" || m1=="" || d1=="" ||　h1=="" || i1=="" || s1=="")
			{
				layer.msg('请选择出门日期');			
			}
			else
			{
				$.ajax({url:"<?php echo http_url();?>admin/members/door_out_sub/<?php echo $result["id"];?>", 
				type: 'POST', 
				data:{y1:y1,m1:m1,d1:d1,h1:h1,i1:i1,s1:s1}, 
				dataType: 'html', 
				timeout: 15000, 
					error: function(){
						layer.closeAll();
						form_loads=1;
						layer.alert('抱歉：程序操作过程中出错，请您稍后再试！', {
							icon: 2,
							skin: 'layer-ext-moon'
						})
					},
