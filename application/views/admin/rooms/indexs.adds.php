...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 系统管理 <span class="c-gray en">&gt;</span> 课厅添加 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
<article class="page-container">
	<div class="form form-horizontal" id="form-article-add" action="#">
    	
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-2"><span class="c-red">*</span>大厅名称：</label>
        ...
        <div class="row cl" style="display:none;">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="100" placeholder="" id="count" name="count">
			</div>
		</div>
		...
		<div class="row cl">
			<div class="col-xs-8 col-sm-9 col-xs-offset-4 col-sm-offset-2">
				<button onClick="article_save_submit();" class="btn btn-primary radius" type="submit"><i class="Hui-iconfont">&#xe632;</i> 添加一个课厅信息 </button>
...
<script>
	...
	function article_save_submit()
	{
		...
		if(form_loads==1)
		{
			...
			var count=$("#count").val().replace(/(^\s*)|(\s*$)/g,"");


			
			if(name=="")
			{
				layer.tips('请填写课厅名称', '#name', {
				  tips: 3
				});			
			}
			else if(count=="")
			{
				layer.tips('请填写课厅容纳人数', '#count', {
				  tips: 3
				});
			}
			else if(parseInt(count)<=0 || parseInt(count)>=9999)
			{
				layer.tips('课厅容纳人数必须在1-9999人之间', '#count', {
				  tips: 3
				});
			}			
			else
			{
				$.ajax({url:"<?php echo http_url();?>admin/rooms/adds_subs", 
				type: 'POST', 
					...
					success:function(result){
						...
						if(result.indexOf("|")>0){
							...
							if(arr[0]==10000){
								...
								setTimeout("location='<?php echo http_url();?>admin/rooms/indexs'",1500);
							}else if(arr[0]==20000){
