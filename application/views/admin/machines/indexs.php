...
<script>
	...
	{
		//alert(1);
		$.ajax({url:"<?php echo http_url();?>admin/machines/indexs_ajaxs/" + "/" + pageindex + "?keywords=" + keywords + "&id=" + id, 
			type: 'POST', 
			dataType: 'html', 
			timeout: 15000, 
			error: function(){
	...
	function edits(id)
	{
		location='<?php echo http_url();?>admin/machines/edits/' + id + '?id=<?php echo $id;?>&keywords=<?php echo $keywords;?>&pageindex=<?php echo $pageindex;?>';
	}
	
	function ups(id,type)
	{
		if(keywords!=""){
			layer.msg('检索模式下不可以使用此功能');
		}
		else
		{	
			if(form_status==0 || form_status==3){
				$.ajax({url:"<?php echo http_url();?>admin/machines/index_ups?id=" + id + "&type=" + type, 
				type: 'POST', 
				dataType: 'html', 
				timeout: 15000, 
					error: function(){
	...
	function downs(id,type)
	{
		...
		else
		{
			//alert(id + "\n" + type);
			if(form_status==0 || form_status==3){
				$.ajax({url:"<?php echo http_url();?>admin/machines/index_downs?id=" + id + "&type=" + type, 
				type: 'POST', 
				dataType: 'html', 
				timeout: 15000, 
					error: function(){
	...
	function del_rs(id){
		if(form_status==0 || form_status==3){
			$.ajax({url:"<?php echo http_url();?>admin/machines/all_subs", 
			type: 'POST', 
			data:{id:id},
			dataType: 'html', 
			timeout: 15000, 
...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> <?php echo $result["name"];?> <span class="c-gray en">&gt;</span> 器材列表 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
<div class="page-container">
	<div class="text-c"> 
    <strong>“<?php echo $result["name"];?>”</strong>分类器材
    &nbsp;&nbsp;
		<input type="text" class="input-text" style="width:250px" placeholder="请输入器材名称、描述词" id="keys" name="keys">&nbsp;&nbsp;
		<button type="submit" class="btn btn-success radius" id="" name="" onClick="soso();"><i class="Hui-iconfont">&#xe665;</i> 搜器材</button>
	</div>
    <span id="inners">
