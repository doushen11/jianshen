...
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 操课管理 <span class="c-gray en">&gt;</span> 操课修改 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
<article class="page-container">
	<div class="form form-horizontal" id="form-article-add" action="#">
       ...
       <div class="row cl">
        ...
        <div class="row cl">
			<label class="form-label col-xs-4 col-sm-2"><span class="c-red">*</span>操课最后安排时间：</label>
			<div class="formControls col-xs-8 col-sm-9">
...
$(document).ready(function() {
	...
	function show_c1()
	{
			...
			<select id="classs" name="classs" onChange="show_c1();">
                ...
                <?php
                	...
                	foreach($query1->result_array() as $array){
						$text='"class":"'.$array["id"].'",';	
						$text1='"class":'.$array["id"].',';
				?>
                <?php if(substr_count($result["money_desc"],$text)>0 || substr_count($result["money_desc"],$text1)>0){?>
                

                
                <option value="<?php echo $array["id"];?>"  <?php if($result["class_id"]==$array["id"]){ ?> selected<?php }?>><?php echo $array["name"];?></option>
                <?php }?>
                <?php }?>
            </select>
			</div>
        ...
        <div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
			<input type="text" class="input-text" value="" placeholder="" id="min" name="min" disabled >
			</div>
		</div> 
	<script>
        function show_c2()
        ...
        <div class="row cl">
			...
			<select id="rooms" name="rooms" onChange="show_c1();">
                ...
                <option value="<?php echo $array["id"];?>" <?php if($result["room_id"]==$array["id"]){ ?> selected<?php }?>><?php echo $array["name"];?></option>
        ...
        <div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
			<input type="text" class="input-text" value="" placeholder="" id="max" name="max" disabled >
			</div>
		</div>       
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				...
				<input type="text" onfocus="WdatePicker({minDate:'#F{$dp.$D(\'datemin\')}'})" id="datemax" class="input-text Wdate" style="width:120px;" value="<?php echo $result["date"];?>" >
			</div>
		</div>
<script>
	function show_times()
			...
			<div class="formControls col-xs-8 col-sm-9">
				<select id="s" name="s" onChange="show_times();">
                	<?php
                    	for($a=9;$a<=21;$a++){
					?>
                	<option value="<?php echo $a;?>" <?php if($a==date("H",$result["start_time"])){?> selected<?php }?>><?php echo $a;?>:00</option>
                    <?php
						}
					?>
                </select>
                ...
                <span id="times_inner">
                <select id="e" name="e">
                	<option value="<?php echo date("H",$result["end_time"]);?>"><?php echo date("H",$result["end_time"]);?>:00</option>
                </select>
                </span>
			</div>
		</div>
	...
	function show_a()
	{
        ...
        <div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<select id="frees" name="frees" onChange="show_a();">
                    ...
                    <option value="1" <?php if($result["sale"]>0){?> selected<?php }?>>正常计费</option>
                    <option value="2" <?php if($result["sale"]==0){?> selected<?php }?>>免费课程</option>
                </select>
			</div>
		</div>
        
        <div class="row cl" id="frees_inner" <?php if($result["sale"]>0){?>style="display:none;"<?php }?>>
			<label class="form-label col-xs-4 col-sm-2"><span class="c-red">*</span>每人收费金额：</label>
			<div class="formControls col-xs-8 col-sm-9">
			<input type="text" class="input-text" value="<?php echo $result["moneys"];?>" placeholder="" id="moneys" name="moneys" style="width:50%"> &nbsp;&nbsp;&nbsp; <font color="#FF0000">线下结算使用</font>
			</div>
		</div>
		...
		<div class="row cl">
			<div class="col-xs-8 col-sm-9 col-xs-offset-4 col-sm-offset-2">
				<button onClick="article_save_submit();" class="btn btn-primary radius" type="submit"><i class="Hui-iconfont">&#xe632;</i> 修改操课信息 </button>
...
<script>
	...
	function article_save_submit()
	{
		...
		if(form_loads==1)
		{
			...
			{
				$.ajax({url:"<?php echo http_url();?>admin/classs/caokes_edit_subs/<?php echo $result["id"];?>", 
				type: 'POST', 
					...
					success:function(result){
						...
						if(result.indexOf("|")>0){
							...
							if(arr[0]==10000){
								...
								setTimeout("location='<?php echo http_url();?>admin/classs/caokes?keywords=<?php echo @$_GET["keywords"];?>&pageindex=<?php echo @$_GET["pageindex"];?>&start_time=<?php echo @$_GET["start_time"];?>&end_time=<?php echo @$_GET["end_time"];?>&states=<?php echo @$_GET["states"];?>'",1500);
							}else if(arr[0]==20000){
