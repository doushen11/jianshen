...
<script>
	...
	{
		//alert(1);
		$.ajax({url:"<?php echo http_url();?>admin/classs/indexs_ajaxs/" + pageindex + "?keywords=" + keywords + "&start_time=" + start_time + "&end_time=" + end_time, 
			type: 'POST', 
			dataType: 'html', 
			timeout: 15000, 
			error: function(){
	...
	function del_it(id){
		layer.confirm('您确定要删除所选数据吗！如果当前课类已被安排课程或者被教练绑定过，则会删除失败！', {
			btn: ['确定','取消'] //按钮
		}, function(){
			del_rs(id);
		}, function(){
	...
	function del_rs(id){
		if(form_status==0 || form_status==3){
			$.ajax({url:"<?php echo http_url();?>admin/classs/dels", 
			type: 'POST', 
			data:{id:id},
			dataType: 'html', 
			timeout: 15000, 
...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 课程管理 <span class="c-gray en">&gt;</span> 课类设置 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
