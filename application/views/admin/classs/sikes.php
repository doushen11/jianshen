...
<script>
	...
	{
		location='<?php echo http_url();?>admin/classs/sikes_edits/' + id + "?keywords=" + keywords + "&pageindex=" + pageindex + "&start_time=" + start_time + "&end_time=" + end_time + "&states=" + states;	
	}
	...
	{
		//alert(1);
		$.ajax({url:"<?php echo http_url();?>admin/classs/sikes_ajaxs/" + pageindex + "?keywords=" + keywords + "&start_time=" + start_time + "&end_time=" + end_time + "&states=" + states, 
			type: 'POST', 
			dataType: 'html', 
			timeout: 15000, 
			error: function(){
	...
	function del_it(id){
		layer.confirm('警告：您确定要删除所选数据吗！请保证您选择的课程状态为正常预约/系统超时类型，其他类型课程不支持删除！', {
			btn: ['确定','取消'] //按钮
		}, function(){
			del_rs(id);
		}, function(){
	...
	function del_rs(id){
		if(form_status==0 || form_status==3){
			$.ajax({url:"<?php echo http_url();?>admin/classs/sikes_dels", 
			type: 'POST', 
			data:{id:id},
			dataType: 'html', 
			timeout: 15000, 
...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 课程管理 <span class="c-gray en">&gt;</span> 私课列表信息 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
<div class="page-container">
    ...
    <input type="text" onfocus="WdatePicker({maxDate:'#F{$dp.$D(\'datemax\')}'})" id="datemin" class="input-text Wdate" style="width:120px;" value="<?php echo $start_time;?>">
		...
		<input type="text" class="input-text" style="width:250px" placeholder="教练手机号/教练昵称/课程价格/课程节点" id="keys" name="keys" value="<?php echo $keywords;?>">&nbsp;&nbsp;
        ...
        <select id="states" name="states" style="height:30px; line-height:30px; border:#e4e4e4 1px solid; width:130px;">
            ...
            <option style="color:green;" value="2" <?php if($states==2){?> selected<?php }?>>正常预约中</option>
            <option style="color:#cc0000;" value="3" <?php if($states==3){?> selected<?php }?>>已被预约了</option>
            <option style="color:#1859a5;" value="4" <?php if($states==4){?> selected<?php }?>>系统超时取消</option>
            <option style="color:#333;" value="6" <?php if($states==6){?> selected<?php }?>>已经完成</option>
        </select>
        &nbsp;&nbsp;
