...
<script>
	...
	{
		location='<?php echo http_url();?>admin/worker/index_edits/' + id + "?keywords=" + keywords + "&pageindex=" + pageindex + "&start_time=" + start_time + "&end_time=" + end_time + "&states=" + states + "&doors=" + doors;	
	}
	
	//会员添加
	function member_add()
	{
		location='<?php echo http_url();?>admin/worker/indexs_add/' + "" + "?keywords=" + keywords + "&pageindex=" + pageindex + "&start_time=" + start_time + "&end_time=" + end_time + "&states=" + states + "&doors=" + doors;	
	}	
	...
	{
		//alert(1);
		$.ajax({url:"<?php echo http_url();?>admin/worker/indexs_ajaxs/" + pageindex + "?keywords=" + keywords + "&start_time=" + start_time + "&end_time=" + end_time + "&states=" + states + "&doors=" + doors, 
			type: 'POST', 
			dataType: 'html', 
			timeout: 15000, 
			error: function(){
	...
	{
		if(form_status==0 || form_status==3){
			$.ajax({url:"<?php echo http_url();?>admin/worker/changes", 
			type: 'POST', 
			data:{id:id},
			dataType: 'html', 
			timeout: 15000, 
	...
	function del_rs(id){
		if(form_status==0 || form_status==3){
			$.ajax({url:"<?php echo http_url();?>admin/worker/index_dels", 
			type: 'POST', 
			data:{id:id},
			dataType: 'html', 
			timeout: 15000, 
...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> sos管理 <span class="c-gray en">&gt;</span> 工作人员列表 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
<div class="page-container">
    ...
    <input type="text" onfocus="WdatePicker({maxDate:'#F{$dp.$D(\'datemax\')||\'%y-%M-%d\'}'})" id="datemin" class="input-text Wdate" style="width:120px;" value="<?php echo $start_time;?>">
		...
		<input type="text" class="input-text" style="width:250px" placeholder="工作人员ID/工作人员手机号/工作人员备注" id="keys" name="keys" value="<?php echo $keywords;?>">&nbsp;&nbsp;
