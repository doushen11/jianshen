...
<script>
	...
	{
		//alert(1);
		$.ajax({url:"<?php echo http_url();?>admin/others/shufs_ajaxs/" + pageindex + "?acts=" + acts, 
			type: 'POST', 
			dataType: 'html', 
			timeout: 15000, 
			error: function(){
	...
	function del_rs(id){
		if(form_status==0 || form_status==3){
			$.ajax({url:"<?php echo http_url();?>admin/others/shufs_dels", 
			type: 'POST', 
			data:{id:id},
			dataType: 'html', 
			timeout: 15000, 
...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 系统管理 <span class="c-gray en">&gt;</span> 轮播图设置 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:show_pages(1);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
<div class="page-container">
	<div class="text-c"> 
    <select id="act" name="act" style="height:32px; line-height:32px; width:100px; border:#CCC 1px solid; font-size:14px;">
    	<option value="1" <?php if($act==1){?> selected<?php }?>>客户端</option>
        <option value="2" <?php if($act==2){?> selected<?php }?>>教练端</option>
    </select>
    &nbsp;&nbsp;
