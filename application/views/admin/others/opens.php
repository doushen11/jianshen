...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 系统管理 <span class="c-gray en">&gt;</span> 进门价格列表 <a id="shuaxins" class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i id="wysx" class="Hui-iconfont">&#xe68f;</i></a></nav>
<div class="page-container">

	<div class="cl pd-5 bg-1 bk-gray mt-20"> <span class="l"><a href="javascript:add_one();"  class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加一条</a></span> &nbsp;&nbsp;&nbsp; <button onClick="save_submit();" class="btn btn-primary radius" type="submit" style="background:#009933; border:#009933 1px solid;"><i class="Hui-iconfont">&#xe632;</i> 更新对应信息 </button> &nbsp;&nbsp;&nbsp; <strong style="color:#FF0000;">* 终点充值金额填写0元则代表上不封顶，对应数据请谨慎填写</strong></div>
	<table class="table table-border table-bordered table-bg">
		<thead>
			<tr>
				<th scope="col" colspan="6">进门价格列表</th>
			</tr>
			<tr class="text-c">
				<th width="40">ID</th>
			  <th width="150">起点充值金额</th>
				<th width="142">终点充值金额</th>
				<th width="98">高峰期享受价格</th>
				<th width="130">低峰期享受价格</th>
				<th width="100">操作</th>
			</tr>
		</thead>
		<tbody id="inners">
			...
			<tr class="text-c" id="sx_<?php echo $array["id"];?>">
				<td><?php echo $array["id"];?><input type="hidden" value="<?php echo $array["id"];?>" class="beyond_5"></td>
				<td><input type="text" value="<?php echo $array["min"];?>" class="beyond_1"></td>
				<td><input type="text" value="<?php echo $array["max"];?>" class="beyond_2"></td>
				<td><input type="text" value="<?php echo $array["money_peak"];?>" class="beyond_3"></td>
				<td><input type="text" value="<?php echo $array["money_slack"];?>" class="beyond_4"></td>
...
<script>
	...
	function save_submit()
	{
		...
		$("#inners tr").each(function(){
			...
			var money_peak=$(this).find(".beyond_3").val();
			var money_slack=$(this).find(".beyond_4").val();
			if(id!="" && mins!="" && maxs!="" && money_peak!="" && money_slack!="")
			{
				if(sphinx=="")
				{
					sphinx=id + "{syx}" + mins + "{syx}" + maxs + "{syx}" + money_peak + "{syx}" + money_slack;
				}
				else
				{
					sphinx=sphinx + "|_|" + id + "{syx}" + mins + "{syx}" + maxs + "{syx}" + money_peak + "{syx}" + money_slack;
				}
			}
			else
			{
		...
		if(subs==1)
		{
			if(form_loads==1)
			{
				$.ajax({url:"<?php echo http_url();?>admin/others/opens_subs",
				type: 'POST', 
	...
	function add_one()
	{
		//添加一行
		rowCount++;  
		var newRow='<tr class="text-c" id="sx_' + rowCount + '"><td>暂无<input type="hidden" value="recsons" class="beyond_5"></td><td><input type="text" value="" class="beyond_1"></td><td><input type="text" value="" class="beyond_2"></td><td><input type="text" value="" class="beyond_3"></td><td><input type="text" value="" class="beyond_4"></td><td class="td-manage"><a title="删除" href="javascript:del_it(' + rowCount + ');" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6e2;</i></a></td></tr>'; 
