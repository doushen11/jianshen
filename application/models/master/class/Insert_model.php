<?php
	...
	class Insert_model extends CI_model
	{
		...
		/**
		* 按照提供的 课程id  房间号，日期，获得可用的时间段
		* @date: 2017年2月24日 下午3:41:12
		* @author: 孙子浩   (Steven Sun)
		* @param: $rs
		* @param: $class_id   课程id
		* @param: $room_id    房间号
		* @param: $days       日期： 例如  2017-02-18
		* @return:
		*/
		public function choose_time($rs,$class_id,$room_id,$day){
		    $query=$this->db->query("SELECT id,node from dg_tearch_plans WHERE date = '$day' AND room_id = '$room_id'  ORDER BY node ASC;");
		    ...
		    $end_time = "";
		    for ($i = 7; $i < 22; $i++) {
		        if ($i < 10) {$star_time = "0".$i.":00";}else{$star_time = $i.":00";}
		        $end_time = $i+1;
		        if ($end_time < 10) {$end_time = "0".($end_time).":00";}else{ $end_time = ($end_time).":00";}
		        $ttr[] = array("node"=>$star_time."-".$end_time,"is_use"=>0,"heve_class"=>0,"expired"=>1);
		    }
		    ...
		    $query1 = $this->db->query("SELECT
                                    ...
                                    FROM
                                    	dg_teacher t
                                    LEFT JOIN dg_tearch_plans p ON t.id = p.tid
                                    WHERE
                                    	t.token_app = '{$rs["token_app"]}'
                                    AND p.date = '{$day}'");
		    
		     $list2 = $query1->result_array();
		     $t_time = time();
		     $t_time = $t_time + 3600 * 24;
		    /*设置不可用的时间点*/
		    foreach ($ttr as  &$value) {
		        
		        $_tmp_time = explode("-", $value['node']);
		    
		        if ($t_time < strtotime($day." ".$_tmp_time[0].":00")) {
		            $value['expired'] = "0";
		        }
		        foreach ($list as $v) {
		            if ($value['node'] == $v['node']) {
		                $value['is_use'] = "1";
		            }
		        }
		        foreach ($list2 as $v) {
		...
		public function adds($rs,$class_id,$room_id,$days,$start_time,$end_time)
		{
            ...
            $new_time = explode("-", $start_time);
            $start_time = $new_time[0].":00";
            $end_time = $new_time[1].":00";
