	...
	class Index_model extends CI_model
	{
		...
		public function my_class_aa_date()
		{
			if(isset($_REQUEST["days"]))
			{
				...
				$day_arr=$this->create_week_normal2($days);
			}	
			else
			{
				//$day_arr=$this->create_week_normal();
				$day_arr=$this->create_week_normal2();
			}
		...
		/**
		* 返回未来(包含今天)六天的礼拜日
		* @date: 2017年2月25日 上午10:38:39
		* @author: 孙子浩   (Steven Sun)
		* @param: variable
		* @return:
		*/
		private function create_week_normal2($days=null){
		    ...
		    $arr = array();
		    $_array=array("周日","周一","周二","周三","周四","周五","周六");
		    $a = 0;
		    for ($i = 0; $i < 7; $i++) {
		        $arr[$i]['day'] = date("Y-m-d",$times + 3600*24*$i);
		        $key = date("w",$times + 3600*24*$i);
		        $arr[$i]['week'] = $_array[$key];
		        $arr[$i]['next_day']  = date("Y-m-d",$times + 3600*24*($i+1));
		        $arr[$i]['last_day']  = date("Y-m-d",$times + 3600*24*$i - 3600*24);
		    }
		   return $arr;
		}
		
		//我的私刻详情-新
		public function my_class_aa_item($rs,$id,$uid)
		{  
			...
			if($query->num_rows()>0)
			{
				...
				$querys=$this->db->query("select `u`.`avatar`,`u`.`id` as `uid`,`u`.`mobile`,`u`.`nickname`,`u`.`brithday` from `dg_orders` as `o` inner join `dg_user` as `u` on `o`.`uid`=`u`.`id` where `o`.`state` in (1,2,3,6) and u.id = '$uid' limit 1");
				$result["buy"]=1;
				$result["avatar"]="";
				$result["uid"]="";
				$result["mobile"]="";
		...
		public function my_class_aa($rs)
		{
			...
			{
				$_array[$i]["day"]=$v['day'];
				$_array[$i]["class"]=$this->get_class_aa($v['day'],$rs["id"]);
				$i++;	
			}
			/**
			 * 处理是否结束
			...
			foreach ($_array as $key => &$value) {
			    ...
			    foreach ($value['class'] as &$val) {
			        if ($val['message']['uid'] != "") {
			            /*时间字符拼接*/
    			        $node_time = $val['node1'];
    			        $node_time = explode("-", $node_time);
    			        $node_time = $day." ".$node_time[1].":00";
    			        /*结束时间戳*/
    			        $end_time_stamp = strtotime($node_time);
    			        /*当前时间戳*/
    			        $now_time_stamp = time();
    			        /*时间戳比较*/
    			        if ($now_time_stamp > $end_time_stamp) {
    			            $val['message']['is_end'] = 1;
    			        }
    			        if ($now_time_stamp < $end_time_stamp) {
    			            $val['message']['is_end'] = 0;
    			        }
			        }
			    }
			}
		...
		private function get_class_aa_user($id,$node,$uid)
		{
			...
			if($query->num_rows()>0)
			{
				...
				$_return = array();
				$_return = $this->read_class_aa_user($result["id"],$mobile);
				
				 if(is_array($_return)){
					$_array["user"]=$_return['nickname'];
					$_array["uid"]=$_return['uid'];					 
				 }else{
				     $_array["user"]= "";
				     $_array["uid"]= "";				     
				 }
		...
		{
			$query=$this->db->query("select `o`.`id`,`u`.`mobile`,`u`.`nickname`,u.id as uid from `dg_orders` as `o` inner join `dg_user` as `u` on `o`.`uid`=`u`.`id` where `o`.`class_id`='$id' and `o`.`state` in (1,2,3,6) limit 1");
			...
			else
			{
				...
				$_tmp_array = array();
				$_tmp_array['nickname']  = $result["nickname"];
				$_tmp_array['uid']  = $result["uid"];
				return $_tmp_array;
			}
		}
		/**
		* 根据当前时间创建包含未来7天时间值(包含创建当前)
		* @date: 2017年2月25日 上午10:56:53
		* @author: 孙子浩   (Steven Sun)
		* @param: variable
		* @return:
		*/
		private function create_week2(){
		...
		public function my_class_bb_join_item($rs,$id)
		{
			...
			if($query->num_rows()>0)
			{
				$pagesize=isset($_REQUEST["pagesize"]) && is_numeric($_REQUEST["pagesize"])?intval($_REQUEST["pagesize"]):30;
				$segment= isset($_REQUEST["pageindex"]) && is_numeric($_REQUEST["pageindex"])?intval($_REQUEST["pageindex"]):1;
				$sql="select `u`.`id` as `uid`,`u`.`nickname`,`u`.`avatar`,`u`.`mobile`,`u`.`gender`,`u`.`brithday` from `dg_user` as `u` inner join `dg_orders` as `o` on `u`.`id`=`o`.`uid` where `o`.`act`='2' and `o`.`pid`='$id' and `o`.`state` in (1,2,3) order by `o`.`id` desc ";
				$pagecount= 200;
				$sql=$this->db->page_json($sql,$pagesize,$pagecount,$pageindex,$pageall,$segment);
				$query=$this->db->query($sql);
				$array=array();
		...
		public function my_class_b_joins($rs,$id)
		{
			...
			if($query->num_rows()>0)
			{
				$sql="select `u`.`nickname`,`u`.`avatar`,`o`.`state` from `dg_orders` as `o` left join `dg_user` as `u` on `o`.`uid`=`u`.`id` where `o`.`pid`='$id' and o.state < 5 order by `o`.`id` desc";
				//echo $sql;
				$sql=$this->db->page_json($sql,$pagesize,$pagecount,$pageindex,$pageall,$segment);
				$query=$this->db->query($sql);
				$arrays=array();
