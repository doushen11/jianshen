 ...
 */
require_once(base_url()."/alipay/alipay.config.php");
require_once(base_url()."/alipay/lib/alipay_notify.class.php");
...
if($verify_result) {//验证成功
	...
	//$pay_record = $db->getOneRow("select * from bx_order  where  orderid =  '".$out_trade_no."' and states = 5 and paystate = 1 and isqx = 0");
      ...
      //  }
	  			...
	  			/*$money1 = $this->db->query("select * from `bx_czyouhi` order by recharge asc");
				$qmoney=$money1->result_array();
				switch($paymoney){
					case $paymoney > 0 && $paymoney < $qmoney[0]['recharge']:
					$reward=0;
					break;
					case $paymoney >=$qmoney[0]['recharge'] && $paymoney <  $qmoney[1]['recharge']:
					$reward = $qmoney[0]['reward'];
					break;
					case $paymoney >=$qmoney[1]['recharge']:
					$reward = $qmoney[1]['reward'];
					break;
