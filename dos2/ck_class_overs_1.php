<?php
	...
	while($array=mysql_fetch_assoc($query))
	{
		...
		$sql_tmp = "select id  from dg_orders where state = 2 and act = 2 and pid = {$result['id']}";
...
// 		}
		...
		while($array_tmp = mysql_fetch_assoc($query_tmp)){
		    if(!mysql_query("update `dg_orders` set `state`='3' where and `act`='2' and `id`= {$array_tmp['id']}"))
		    {
		        mysql_query("ROLLBACK");//判断执行失败回滚
		        exit();
		    }
